...
void squareStep(Matrix<int>Array, int x, int y, int reach, unsigned int Width)
{
	int count = 0;
	float avg = 0.0f;
...
void diamondStep(Matrix<int>Array, int x, int y, int reach, unsigned int Width)
{
	int count = 0;
	float avg = 0.0f;
	if (edgeCondition(x, y, reach, Width))
...
void diamondSquare(Matrix<int>Array, int size, unsigned int Width)
{
	int half = size / 2;
	if (half < 1)
		return;
	for (int y = half; y < Width; y += size)
		for (int x = half; x < Width; x += size)
			squareStep(std::move(Array), x % Width, y % Width, half, Width);
	int col = 0;
	for (int x = 0; x < Width; x += half)
	{
		...
		if (col % 2 == 1)
			for (int z = half; z < Width; z += size)
				diamondStep(std::move(Array), x % Width, z % Width, half, Width);
		else
			for (int z = 0; z < Width; z += size)
				diamondStep(std::move(Array), x % Width, z % Width, half, Width);
	}
	diamondSquare(std::move(Array), size / 2, Width);
}
/// </DIAMOND SQUARE>*/